...
fun TreeVisualizerPreview() {
    ...
    var traversalIterator by remember {
        mutableStateOf(
            bsfSequence(
                root,
            ).iterator()
    ...
    var selectedTraversal by remember {
        mutableStateOf(TreeTraversalType.INORDER)
    }
    val onTraversalTypeChanged: (String) -> Unit = {
        ...
        traversalIterator= when (it) {
            ...
            TreeTraversalType.BFS -> bsfSequence(root).iterator()
            TreeTraversalType.DFS -> dfsSequence(root).iterator()
            else -> bsfSequence(root).iterator()
        }
